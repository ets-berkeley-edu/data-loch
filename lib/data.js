 ...
 * @param  {String}       subpath                                                       The folder within the data folder in which to parse the data
 * @param  {Function}     callback                                                      Standard callback function
 * @param  {Object}       callback.enrollments                                          The retrieved enrollments
 * @param  {Number}       callback.enrollments[id].id                                   Unique surrogate id for the enrollment
 * @param  {Number}       callback.enrollments[id].canvas_id                            Primary key for this record in the Canvas enrollments table
 * @param  {Number}       callback.enrollments[id].root_account_id                      Root account id associated with this enrollment
 * @param  {Number}       callback.enrollments[id].course_section_id                    Foreign key to the course section for this enrollment
 * @param  {Number}       callback.enrollments[id].role_id                              Foreign key to the role of the person enrolled in the course
 * @param  {String}       callback.enrollments[id].type                                 Enrollment type: TaEnrollment, DesignerEnrollment, StudentEnrollment, TeacherEnrollment, StudentViewEnrollment, ObserverEnrollment
 * @param  {String}       callback.enrollments[id].workflow_state                       Workflow state for enrollment: active, completed, rejected, deleted, invited, creation_pending
 * @param  {Date}         callback.enrollments[id].created_at                           Timestamp for when this section was entered into the system
 * @param  {Date}         callback.enrollments[id].updated_at                           Timestamp for when the last time the section was updated
 * @param  {Date}         callback.enrollments[id].start_at                             Enrollment start date
 * @param  {Date}         callback.enrollments[id].end_at                               Enrollment end date
 * @param  {Date}         callback.enrollments[id].completed_at                         Enrollment completed date
 * @param  {Boolean}      callback.enrollments[id].self_enrolled                        Enrollment was created via self-enrollment
 * @param  {String}       callback.enrollments[id].sis_source_id                        Id of correlated enrollment in the SIS (assuming the SIS is configured properly)
 * @param  {Number}       callback.enrollments[id].course_id                            Foreign key to course for this enrollment
 * @param  {Number}       callback.enrollments[id].user_id                              Foreign key to user for the enrollment
 * @param  {Number}       callback.enrollments[id].enrollment_term_id                   Foreign key for the attributes of the enrollment
 * @param  {Number}       callback.enrollments[id].course_account_id                    Foreign key to the account of the enrolled course
 * @param  {Number}       callback.enrollments[id].computed_final_score                 Final score for the enrollment
 * @param  {Number}       callback.enrollments[id].computed_current_score               Current score for the enrollment
 */
var getEnrollments = module.exports.getEnrollments = function(subpath, callback) {
  parseData(subpath, {
    'enrollment_dim':  ['id', 'canvas_id', 'root_account_id', 'course_section_id', 'role_id', 'type', 'workflow_state', 'created_at', 'updated_at', 'start_at', 'end_at', 'completed_at', 'self_enrolled',
                        'sis_source_id', 'course_id', 'user_id'],
    'enrollment_fact': ['id', 'user_id', 'course_id', 'enrollment_term_id', 'course_account_id', 'course_section_id', 'computed_final_score', 'computed_current_score']
  }, callback);
};
