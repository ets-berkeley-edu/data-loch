...
var parseDataFiles = module.exports.parseDataFiles = function(file, callback) {
  // Check if the request file(s) have already been parsed
  if (parsedDataFilesCache[file]) {
    return callback(parsedDataFilesCache[file]);
  }

  var rnd = Math.random();
  glob('data/' + file + '*', null, function (err, paths) {
    var data = [];
...
/**
 * Make an API request to the Canvas Redshift REST API
 *
 * @param  {String}             path                    The REST API path
 * @param  {Function}           callback                Standard callback function
 * @param  {Object}             callback.data           The REST API response
 */
var apiRequest = module.exports.apiRequest = function(path, callback) {
  ...
  var url = (HTTPS ? 'https' : 'http') + '://' + HOST + path;
  request({
    'url': url,
    'headers': {
 ...
 */
var downloadFiles = module.exports.downloadFiles = function(files, callback) {
  var filenames = [];

  async.eachSeries(files, function(file, done) {
    var filename = file.filename.split('.').shift();
    filenames.push(filename);
    var path = './data/base/' + filename;
    log.info({'file': filename}, 'Starting Canvas data file download');

    // Don't download the file if it has already been downloaded
    fs.exists(path, function(exists) {
      if (exists) {
        log.info({'file': filename}, 'Skipping Canvas data file download. File already exists');
        return done();
      } else {
        // Ensure that we can catch errors in the download and ungzipping
        var d = domain.create();
        d.on('error', function (err) {
          log.error({'err': err, 'file': filename}, 'Unable to download Canvas data file');
          return done();
        });
        d.run(function() {
          var start = Date.now();
          var stream = request(file.url).pipe(zlib.createGunzip()).pipe(fs.createWriteStream(path));
